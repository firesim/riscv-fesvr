...
void htif_hexwriter_t::read_chunk(reg_t taddr, size_t len, void* vdst)
{
...
void htif_hexwriter_t::write_chunk(reg_t taddr, size_t len, const void* vsrc)
{
  assert(len % chunk_align() == 0);
  assert(taddr < width*depth);
  assert(taddr+len <= width*depth);
...
std::ostream& operator<< (std::ostream& o, const htif_hexwriter_t& h)
{
  ...
  {
    std::map<reg_t,std::vector<char> >::const_iterator i = h.mem.find(addr);
    if(i == h.mem.end())
